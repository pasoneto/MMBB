...
def createBeat(segundosSegment, beats, shiftN, beat, name, repetitions):
    ...
    bar.export(f"./example/{name}_{shiftN}.mp3", format="mp3")

beat = AudioSegment.from_wav("./beatUnit.wav")

sr = 75 #Sampling rate used to measure nSamples of excerpt
beats = 6*4 #How many beats happen throughout all of the segment
...
repetitions = beats*1 #How many repetitions for each segment should the output have
